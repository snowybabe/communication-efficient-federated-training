...
\begin{align}
\nonumber
\min_{\y\in\RR^d} \underbrace{\lrnorm{\r - \bLambda \x_t}_1 + \frac{\lrnorm{\y - \lrincir{\x_t - \eta_t\g_t^{(n)}}}^2}{2\eta_t\gamma}}_{=: g(\r, \y)},
\end{align} subject to:
\begin{align}
\nonumber
\r = \bLambda \y.
...
\begin{align}
\nonumber
L(\r, \y, \bomega) = g(\r, \y) + \lrangle{\bomega, \r - \bLambda \y} + \frac{\rho}{2}\lrnorm{\r - \bLambda \y}^2.
...
= & \argmin_{\r\in\RR^d} \lrnorm{\r - \bLambda \x_t}_1 + \lrangle{\bomega_j, \r} + \frac{\rho}{2}\lrnorm{\r - \bLambda \y_j}^2 \\ \nonumber
= & \argmin_{\r\in\RR^d} \lrnorm{\r - \bLambda \x_t}_1 + \frac{\rho}{2}\lrnorm{\r - \lrincir{\bLambda \y_j - \frac{1}{\rho}\bomega_j}}^2 \\ \nonumber
= & \Prox_{\rho, \lrnorm{\cdot - \bLambda \x_t}_1}\lrincir{\bLambda \y_j - \frac{1}{\rho}\bomega_j} \\ \nonumber
= & \Prox_{\rho, \lrnorm{\cdot}_1}\lrincir{\bLambda \y_j - \frac{1}{\rho}\bomega_j - \bLambda \x_t}  + \bLambda \x_t \\ \nonumber
= & \left[\bLambda (\y_j - \x_t) \mathrm{-} \frac{\bomega_j}{\rho} \mathrm{-} \rho\right]_\mathrm{+}  \mathrm{-} \left[\bLambda (\x_t - \y_j) \mathrm{+} \frac{\bomega_j}{\rho} \mathrm{-} \rho\right]_\mathrm{+}  \mathrm{+} \bLambda \x_t.
\end{align}
...
= & \argmin_{\y\in\RR^d} \frac{\lrnorm{\y - \lrincir{\x_t - \eta_t\g_t^{(n)}}}^2}{2\eta_t\gamma} - \lrangle{\bomega_j, \bLambda \y} + \frac{\rho}{2}\lrnorm{\r_{j+1} - \bLambda \y}^2 \\ \nonumber
= & \argmin_{\y\in\RR^d} \lrnorm{\bLambda \y - \lrincir{\r_{j+1} + \frac{\bomega_j}{\rho}}}^2 + \frac{\lrnorm{\y - \lrincir{\x_t - \eta_t\g_t^{(n)}}}^2}{\rho\eta_t\gamma} \\ \nonumber
 = & \Prox_{\frac{2}{\rho\eta_t\gamma}, \lrnorm{\bLambda \cdot - \lrincir{\r_{j+1} + \frac{\bomega_j}{\rho}}}^2} \lrincir{\x_t - \eta_t\g_t^{(n)}} \\ \nonumber
= & \lrincir{\frac{4}{\rho\eta_t\gamma}\bLambda^\top\bLambda + \I}^{-1}\lrincir{\x_t - \eta_t\g_t^{(n)}+\frac{4}{\rho\eta_t\gamma}\lrincir{\r_{j+1} + \frac{\bomega_j}{\rho}}}.
\end{align}

\textbf{Update of $\bomega$.} Given $\r_{j+1}$ and $\y_{j+1}$, $\bomega_{j+1}$ is updated by the following rule.
\begin{align}
\nonumber
\bomega_{j+1} = \bomega_j + \rho\lrincir{\r_{j+1} - \bLambda\y_{j+1}}.
\end{align}

\begin{algorithm}[!]
    \caption{Communication Efficient Update of $\nabla_{t+1}^{(n)}$.}
    ...
    \begin{algorithmic}[1]
        \Require given $\x_t$.
           \For {$j=0,1,2, ..., J-1$}
            ...
            \begin{align}
            \nonumber
            \r_{j+1} = \left[\bLambda (\y_j - \x_t) \mathrm{-} \frac{\bomega_j}{\rho} \mathrm{-} \rho\right]_\mathrm{+}  \mathrm{-} \left[\bLambda (\x_t - \y_j) \mathrm{+} \frac{\bomega_j}{\rho} \mathrm{-} \rho\right]_\mathrm{+}  \mathrm{+} \bLambda \x_t.
            \end{align}
            \State Update $\y_{j+1}$ by performing:
            \begin{align}
	   \nonumber
	   \y_{j+1} = \lrincir{\frac{4}{\rho\eta_t\gamma}\bLambda^\top\bLambda + \I}^{-1}\lrincir{\x_t - \eta_t\g_t^{(n)}+\frac{4}{\rho\eta_t\gamma}\lrincir{\r_{j+1} + \frac{\bomega_j}{\rho}}}.
	   \end{align} 
